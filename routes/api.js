  ...
  app
    ...
    .put(async (req, res) => {
      ...
      try {
        ...
        await Thread.findByIdAndUpdate(report_id, query, {
          runValidators: true,
        });
        console.log("reported Thread", report_id)
  ...
  app
    ...
    .get(async (req, res) => {
      ...
      try {
        ...
        console.log("get replies", thread_id)
        res.json(threadData);
      } catch (error) {
        console.error(error.message);
      }
    ...
    .post(async (req, res) => {
      ...
      try {
        ...
        const updatedThread = await Thread.findByIdAndUpdate(thread_id, query, {
          new: true,
          runValidators: true,
        });
        const replies = await Reply.find({ threadId: thread_id }).exec();

        const threadData = getThreadData(updatedThread, replies);
        res.json(threadData);
      } catch (error) {
        console.error(error.message);
      }
    ...
    .put(async (req, res) => {
      ...
      try {
        ...
        const replyQuery = { reported: true, updatedAt: new Date() };
        const newReply = await Reply.findByIdAndUpdate(reply_id, replyQuery, {
          runValidators: true,
          new: true,
        });

        console.log("reported reply", reply_id+" reply", thread_id+" thread")
        res.send(MESSAGE.REPORTED);
      } catch (error) {
        console.error(error.message);
      }
    ...
    .delete(async (req, res) => {
      ...
      try {
        ...
        const replyQuery = { text: DELETED_TEXT, updatedAt: new Date() };
        const newReply = await Reply.findByIdAndUpdate(reply_id, replyQuery, {
          runValidators: true,
          new: true,
        });

        console.log("deleted reply", reply_id+" reply", thread_id+" thread")
        res.send(MESSAGE.SUCCESS);
      } catch (error) {
        console.error(error.message);
      }
